 ...
 * ╔══════════════════════════════════════════════════════════════╗
 * ║  🧪 测试代码区 — 自动关灯功能验证                             ║
 * ║  目的：自动开灯 5 秒后强制 PIR 和声音都返回"未检测到"，       ║
 * ║        从而触发超时关灯逻辑，无需实际传感器配合。              ║
 * ║  正式部署前请删除本区域全部代码。                             ║
 * ╚══════════════════════════════════════════════════════════════╝
 * ================================================================ */
#define TEST_AUTO_OFF_DELAY_MS  5000U   /* 测试：开灯后多少 ms 强制传感器返回 0 */
static uint32_t test_auto_on_tick = 0U; /* 测试：自动开灯时刻的时间戳 */
/* ================================================================
 * 🧪 测试代码区结束
 * ================================================================ */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    default:
        ...
        /* ============================================================
         * 🧪 测试：自动开灯 TEST_AUTO_OFF_DELAY_MS 后强制传感器返回 0
         * 目的：模拟"人离开且安静"，触发超时关灯，验证自动关灯逻辑。
         *
         * 工作方式：
         *   开灯后开始计时，满 5 秒后强制 PIR=无人、声音=安静，
         *   同时将 last_activity_tick 设为过期值，使超时判定立即成立，
         *   下一轮循环即触发关灯（无需再等 5 秒）。
         *
         * 正式部署前请删除此段代码。
         * ============================================================ */
        if (auto_light_on == 1U &&
            test_auto_on_tick != 0U &&
            ((HAL_GetTick() - test_auto_on_tick) > TEST_AUTO_OFF_DELAY_MS))
        {
            pir_detected = PIR_NOT_DETECTED;   /* 强制：无人 */
            snd_detected = SOUND_NOT_DETECTED;  /* 强制：安静 */
            /*
             * 将活动时间戳设为过期值（AUTO_OFF_TIMEOUT_MS + 100ms 之前），
             * 确保本轮循环末尾的超时判定立即成立，无需再等待。
             */
            last_activity_tick = HAL_GetTick() - AUTO_OFF_TIMEOUT_MS - 100U;
        }
        /* ============================================================
         * 🧪 测试代码结束
         * ============================================================ */
        ...
        if (auto_light_on == 0U)
        {
            ...
            if (pir_detected == PIR_DETECTED)
            {
                ...
                /* 🧪 测试：记录自动开灯时刻，用于 5 秒后强制关灯 */
                test_auto_on_tick = HAL_GetTick();
